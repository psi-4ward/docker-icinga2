...
foreach my $l_key (oid_lex_sort(keys(%{$hdskIOTableDiff}))){
  ...
  $perfdata=sprintf("Read=%.2fMB Write=%.2fMB Read=%.2fMB Write=%.2fMB Total=%.2fMB;%d;%d",$IONReadMBsec,$IONWrittenMBsec,$IOReadssec,$IOWritessec,$dskIOTotal,$opt_warn,$opt_crit);
